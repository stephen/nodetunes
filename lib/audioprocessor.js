...
AudioProcessor.prototype.makeMagicCookie = function(){
  ...
  var arrCookie = [
    '0x00','0x00','0x01','0x60', //352 frames per seconds
    '0x00', //compatible: 0
    '0x10', //bits depth: 16
    '0x28', //unused: 40
    '0x0A', //unused: 10
...
AudioProcessor.prototype.decodeALAC = function(alacData, callback){
  ...
  console.log('packets size: ', alacData.length);
  dec.packets([alacData.length]); //! GET THIS PARAMETER CORRECTLY
  dec.end(alacData);
...
AudioProcessor.prototype.process = function(audio, sequenceNumber) {
  if(this.rtspServer.audioCodec.indexOf('L16') > 0){
    ...
    this.decodeALAC(audio,function(pcm){
      console.log('decoded length: ', pcm.length);
