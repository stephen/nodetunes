...
var server = function() {
	...
	var handler = function(socket) {
		...
		socket.on('data', function(c) {
			...
			if (remaining >= 0) {
				remaining -= c.length;
				console.log('WAITING ON ' + remaining + ' BYTES OF CONTENT REMAINING');
			}
			...
			if (buffer.indexOf(terminator) >= 0) {
				...
				if (!headerData.hasContent) {
					console.log('CALLING ' + headerData.directive + ' WITHOUT CONTENT');
			...
			if (remaining <= 0 && hasContent) {
				console.log('CALLING ' + headerData.directive + ' WITH FOLLOWING CONTENT');
