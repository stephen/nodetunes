...
var txtSetup = {
	...
	tx: '1',			// ?
	ch: '2',			// # channels
	cn: '0,1,2',			// codec; 0=pcm, 1=alac, 2=aac, 3=aac elc; fwiw Sonos supports aac; pcm required for iPad+Spotify
...
getmac.getMac(function(err, macAddress){
	...
	net.createServer(server.handler).listen(5000, function() {
		var ad = mdns.createAdvertisement(mdns.tcp('raop'), 5000, {
			name: "5F513885F785@AirSonos",	// it looks like the mac address matters and AirPlay doesn't care if it's real or not. just need to be consistent
			//name: macAddress.replace(/:/g, '') + "@AirSonos",
			txtRecord: txtSetup
		});
