...
module.exports = function(rtspServer) {
	...
	var setup = function(response, headers) {
		...
		response.setOK(headers['CSeq']);
		response.addHeader('Transport', 'RTP/AVP/UDP;unicast;mode=record;server_port=' + rtspServer.audioPort + ';control_port=' + rtspServer.controlPort + ';timing_port=' + rtspServer.timingPort);
